module.exports = (app, client) =>{
    ...
    app.get('/loadAboutTranslation',(req,res)=>{
        client.query(`SELECT content FROM about_translation WHERE id = 1`,(err,response)=>{
            if(err){
                DbErrorHandler(res,"could not get about translation from DB");
            }else{
                sendAbouteHandler(res,response);
            }
        });
    ...
    app.put('/editAboutTranslation',(req,res)=>{
        const newContent = req.body.data.content;
        const query = editAboutTranslationQuery(newContent);
        client.query(query,(err,response)=>{
            if(err){
                DbErrorHandler(res,"could not edit about page");
            }else{
...
const editAboutTranslationQuery = (newContent) =>{
    return `UPDATE about_translation SET content ='${newContent}' WHERE id = 1 RETURNING content`;
}